...
export async function dataCentralizationRuntime({
	...
	const closeChallengeListener = listenForChallenges(async (challengeNumber: bigint, challenge: Challenge, event?: any) => {
		...
		const slackStartMessage = `Starting pool sync`;
		...
		const slackMessage = `Finished pool sync for ${updatedPools.length} pools in ${totalSeconds}ms. Graph update took ${totalGraphSeconds}ms. Mongo insert took ${totalMongoSeconds}ms.`;
