...
export async function findMissedAssertion(): Promise<number | null> {
    ...
    //TODO this is for being able to switch from testnet to mainnet, once the switch is done we should only be using the config rpc
    const rollupProvider = new ethers.JsonRpcProvider("https://arb-goerli.g.alchemy.com/v2/WNOJEZxrhn3a0PzKUVEZgeRJqxOL7brv");
    const blockRangePerRequest = 500000;
