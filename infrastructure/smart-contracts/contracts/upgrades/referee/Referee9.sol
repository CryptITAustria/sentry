...
contract Referee9 is Initializable, AccessControlEnumerableUpgradeable {
    ...
    // Boost rates to be applied at next halving event
    uint256[] public stagedTierBoostFactors;

    // The lower supply bound to trigger a reverse halving of emission rewards
    uint256 public reverseHalvingThreshold;

    // The upper supply bound to trigger a halving of emission rewards
    ...
    // TODO Talk to Chris about adding additional field(s) in the next two events for claiming node license Id?
    ...
    function initialize() public reinitializer(6) {
        ...
        _initBoostFactors[0] = 150;
        _initBoostFactors[1] = 200;
        _initBoostFactors[2] = 300;
        _initBoostFactors[3] = 700;

        _updateRewardTierPercentages(_initTierPercentageThresholds, _initBoostFactors, false);

        // Set Initial Halving Thresholds
        reverseHalvingThreshold = 0;
        halvingThreshold = 1_250_000_000 * 10 ** 18;

        // Confirm the current total supply is between the reverse halving and halving thresholds
        require(reverseHalvingThreshold < getCombinedTotalSupply() && halvingThreshold > getCombinedTotalSupply(), "50");
    ...
    //TODO - review all new functions below this line
    ...
    */
    function _executeHalving(uint256 totalSupply) private {
        ...
        emit HalvingEvent(challengeCounter, totalSupply, halvingThreshold);
    }   
    ...
    */
    ...
    /**
     * @notice Updates the reward tier staking thresholds and boost factors.
     * @dev This function is only executed upon halving events.     
    */
